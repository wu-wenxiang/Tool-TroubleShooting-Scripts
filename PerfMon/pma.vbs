...
'
'
' Author: Jeff Fanjoy, VMC Consulting (v-11jfan@mssupport.microsoft.com / JeffreyFan@vmc.com)
...
Const pma_Version = "v1.10"
Const pma_Author = "Jeff Fanjoy, VMC Consulting (v-11jfan / JeffreyFan@vmc.com)"
...
Dim ArrNICs()              ' Array for stornig NIC details
...
Dim bShowTop10_Handle      ' Whether or not to show the TOP 10: Handle Count
Dim bShowTop10_Thread      ' Whether or not to show the TOP 10: Thread Count
Dim bShowTop10_PBytes      ' Whether or not to show the TOP 10: Private Bytes
Dim bShowTop10_VBytes      ' Whether or not to show the TOP 10: Virtual Bytes
Dim bShowTop10_WSet        ' Whether or not to show the TOP 10: Working Set
Dim bShowTop10_CPU         ' Whether or not to show the TOP 10: % Processor Time
Dim bShowTop10_IOData      ' Whether or not to show the TOP 10: IO Data Bytes
...
Dim bIncludeTop10_Handle   ' Whether or not to Include the TOP 10: Handle Count
Dim bIncludeTop10_Thread   ' Whether or not to Include the TOP 10: Thread Count
Dim bIncludeTop10_PBytes   ' Whether or not to Include the TOP 10: Private Bytes
Dim bIncludeTop10_VBytes   ' Whether or not to Include the TOP 10: Virtual Bytes
Dim bIncludeTop10_WSet     ' Whether or not to Include the TOP 10: Working Set
Dim bIncludeTop10_CPU      ' Whether or not to Include the TOP 10: % Processor Time
Dim bIncludeTop10_IOData   ' Whether or not to Include the TOP 10: IO Data Bytes
...
If Left(UCase(getOS()),4) = "WIN7" Then
...
bSystemUpTime = False
bShowTop10_Handle = False
bShowTop10_Thread = False
bShowTop10_PBytes = False
bShowTop10_VBytes = False
bShowTop10_WSet = False
bShowTop10_CPU = False
bShowTop10_IOData = False
bShowMemory = False
bShowProcessor = False
bShowDisk = False
bShowNIC = False
bIncludeTop10_Handle = False
bIncludeTop10_Thread = False
bIncludeTop10_PBytes = False
bIncludeTop10_VBytes = False
bIncludeTop10_WSet = False
bIncludeTop10_CPU = False
bIncludeTop10_IOData = False
...
Sub SetRuntimeOptions(strPrefix, strTitle, strDefault)
   ...
   strInputMsg = strInputMsg & "H = TOP 10 Processes: Handle Count" & vbCrLf
   strInputMsg = strInputMsg & "S = TOP 10 Processes: Thread Count" & vbCrLf
   strInputMsg = strInputMsg & "B = TOP 10 Processes: Private Bytes" & vbCrLf
   strInputMsg = strInputMsg & "V = TOP 10 Processes: Virtual Bytes" & vbCrLf
   strInputMsg = strInputMsg & "W = TOP 10 Processes: Working Set" & vbCrLf
   strInputMsg = strInputMsg & "C = TOP 10 Processes: % Processor Time" & vbCrLf
   strInputMsg = strInputMsg & "I = TOP 10 Processes: IO Data Bytes/sec" & vbCrLf & vbCrLf
   ...
   strInputMsg = strInputMsg & "   HSBVWCI = All Top 10 Processes only." & vbCrLf
   ...
   if instr(strInput, "H") > 0 then bIncludeTop10_Handle = True
   if instr(strInput, "S") > 0 then bIncludeTop10_Thread = True
   if instr(strInput, "B") > 0 then bIncludeTop10_PBytes = True
   if instr(strInput, "V") > 0 then bIncludeTop10_VBytes = True
   if instr(strInput, "W") > 0 then bIncludeTop10_WSet = True
   if instr(strInput, "C") > 0 then bIncludeTop10_CPU = True
   if instr(strInput, "I") > 0 then bIncludeTop10_IOData = True
   ...
   if instr(strInput, "*") > 0 then
      ...
      bIncludeNIC = True
      bIncludeTS = True
      bIncludeTop10_Handle = True
      bIncludeTop10_Thread = True
      bIncludeTop10_PBytes = True
      bIncludeTop10_VBytes = True
      bIncludeTop10_WSet = True
      bIncludeTop10_CPU = True
      bIncludeTop10_IOData = True
   end if
...
Sub ParseCommandLine()
   ...
   if WScript.Arguments.Count = 0 then
      ...
      strOrgFile = WScript.Arguments.Item(0)
...
Sub Build_RecordSets()
   ...
   With objRSCounters
      ...
      if bIncludeMemory then
         ...
         if not bIncludeTop10_Handle then .AddNew "Counter", "\Process(_Total)\Handle Count"
         if not bIncludeTop10_Thread then .AddNew "Counter", "\Process(_Total)\Thread Count"
         if not bIncludeTop10_PBytes then .AddNew "Counter", "\Process(_Total)\Private Bytes"
         if not bIncludeTop10_VBytes then .AddNew "Counter", "\Process(_Total)\Virtual Bytes"
         if not bIncludeTop10_WSet then .AddNew "Counter", "\Process(_Total)\Working Set"
      end if
      if bIncludeTop10_Handle then .AddNew "Counter", "\Process(*)\Handle Count"
      if bIncludeTop10_Thread then .AddNew "Counter", "\Process(*)\Thread Count"
      if bIncludeTop10_PBytes then .AddNew "Counter", "\Process(*)\Private Bytes"
      if bIncludeTop10_VBytes then .AddNew "Counter", "\Process(*)\Virtual Bytes"
      if bIncludeTop10_WSet then .AddNew "Counter", "\Process(*)\Working Set"
      ...
      if bIncludeTop10_CPU then .AddNew "Counter", "\Process(*)\% Processor Time"
      ...
      if bIncludeTop10_IOData then .AddNew "Counter", "\Process(*)\IO Data Bytes/sec"
...
Sub IdentifyConcerns()
   ...
   While not objRSResults.EOF
      ...
      if (objRSResults("Counter") = "\Memory\Pool Paged Bytes") then
      ...
      if (objRSResults("Counter") = "\Memory\Pool NonPaged Bytes") then
      ...
      if objRSResults("Counter") = "\Memory\Cache Bytes" then
...
Sub ParseOutputFile(strFile)
   ...
   For i = 1 to iNumCounters
      ...
      if instr(ArrHeaders(i), "\Process(") > 0 AND _
         ...
         ArrHeaders(i) <> "\Process(_Total)\Handle Count" then bShowTop10_Handle = True
      if instr(ArrHeaders(i), "\Process(") > 0 AND _
         instr(ArrHeaders(i),"\Thread Count") > 0 AND _
         ArrHeaders(i) <> "\Process(_Total)\Thread Count" then bShowTop10_Thread = True
      if instr(ArrHeaders(i), "\Process(") > 0 AND _
         instr(ArrHeaders(i),"\Private Bytes") > 0 AND _
         ArrHeaders(i) <> "\Process(_Total)\Private Bytes" then bShowTop10_PBytes = True
      if instr(ArrHeaders(i), "\Process(") > 0 AND _
         instr(ArrHeaders(i),"\Virtual Bytes") > 0 AND _
         ArrHeaders(i) <> "\Process(_Total)\Virtual Bytes" then bShowTop10_VBytes = True
      if instr(ArrHeaders(i), "\Process(") > 0 AND _
         instr(ArrHeaders(i),"\Working Set") > 0 AND _
         ArrHeaders(i) <> "\Process(_Total)\Working Set" then bShowTop10_WSet = True
      if instr(ArrHeaders(i), "\Process(") > 0 AND _
         instr(ArrHeaders(i),"\% Processor Time") > 0 AND _
         ArrHeaders(i) <> "\Process(_Total)\% Processor Time" then bShowTop10_CPU = True
      if instr(ArrHeaders(i), "\Process(") > 0 AND _
         instr(ArrHeaders(i),"\IO Data Bytes/sec") > 0 AND _
         ArrHeaders(i) <> "\Process(_Total)\IO Data Bytes/sec" then bShowTop10_IOData = True
...
'         ShowMsg "   W00t: " & ArrTempData(l-1) & " - " & ArrTempData(l)
   ...
   if bSystemUpTime = True then
      ...
      for i = 0 to iTotalSamples
         ...
         if ArrData(i,iNumCounters) <> "" then iThisUptime = Int(ArrData(i,iNumCounters))
...
'   ' Populate the objRSResults recordset while calculating averages
   ...
   for i = 0 to iTotalSamples
      ...
      for j = 1 to iNumCounters
         ...
         if ArrData(i,j) <> "" then
            if not ((instr(ArrHeaders(j), "% Processor Time") > 0) AND dTmpData > 1000) then
...
Sub WriteResults()
   ...
   if bShowTop10_Handle then
      strMsg = strMsg & vbCrLf & vbCrLf
      
      strMsg = strMsg & CreateDisplayHeader("TOP 10: Handle Count")
      objRSProcess.Sort = "Handle_Max DESC"
      if not objRSProcess.BOF then objRSProcess.MoveFirst
      
      iCount = 1
      While (not objRSProcess.EOF) AND (iCount <= 10)
   ...
   if bShowTop10_Thread then
      strMsg = strMsg & vbCrLf & vbCrLf
      
      strMsg = strMsg & CreateDisplayHeader("TOP 10: Thread Count")
   ...
   if bShowTop10_PBytes then
      strMsg = strMsg & vbCrLf & vbCrLf
      
      strMsg = strMsg & CreateDisplayHeader("TOP 10: Private Bytes")
   ...
   if bShowTop10_VBytes then
      strMsg = strMsg & vbCrLf & vbCrLf
      
      strMsg = strMsg & CreateDisplayHeader("TOP 10: Virtual Bytes")
   ...
   if bShowTop10_WSet then
      strMsg = strMsg & vbCrLf & vbCrLf
      
      strMsg = strMsg & CreateDisplayHeader("TOP 10: Working Set")
   ...
   if bShowTop10_CPU then
      strMsg = strMsg & vbCrLf & vbCrLf
      
      strMsg = strMsg & CreateDisplayHeader("TOP 10: % Processor Time")
   ...
   if bShowTop10_IOData then
      strMsg = strMsg & vbCrLf & vbCrLf
      
      strMsg = strMsg & CreateDisplayHeader("TOP 10: IO Data Bytes")
